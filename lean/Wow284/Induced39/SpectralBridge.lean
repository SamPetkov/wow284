import Wow284.Induced39.FiniteCertificates
import Wow284.Induced39.LDLData
import Mathlib.Analysis.Matrix.PosDef

/-! Exact positive-definiteness bridge for the order-39 graph. -/
namespace Wow284.Induced39
open Matrix
def Dq : Matrix Vertex Vertex ℚ := D.map (Int.castRingHom ℚ)
theorem Mcore_eq_shifted_distance :
    Mcore = (6 : ℚ) • Dq + (35 : ℚ) • (1 : Matrix Vertex Vertex ℚ) := by
  rw [Mcore, Dq, semantic_distance_eq_Dcert]
  ext i j
  change (((6 * Dcert i j + 35 * (if i = j then 1 else 0) : ℤ) : ℚ)) =
    (6 : ℚ) * ((Dcert i j : ℤ) : ℚ) +
      (35 : ℚ) * (if i = j then 1 else 0)
...
theorem shifted_distance_posDef :
    ((6 : ℚ) • Dq + (35 : ℚ) • (1 : Matrix Vertex Vertex ℚ)).PosDef := by
...
theorem real_eigenpair_above_shift {mu : ℝ} {x : Vertex → ℝ}
  ...
  have hprod : 0 < ((6 : ℝ) * mu + 35) * dotProduct x x := by
    nlinarith [hpos]
  have hcoeff : 0 < (6 : ℝ) * mu + 35 := by
