...
**Status:** promoted after independent proof audit and literature review under
Proof Audit 14.  The theorem appears as Theorem `thm:four-to-one` in the
...
\[
 \frac{C_k}{h_k}-\frac{8C_k}{8h_k+13}
 =\frac{13(k+2)(k^2+3)}{6(48k+109)}
 \sim\frac{13}{288}k^2.
\]
...
If \(X\) is cocktail-party, then \(\varepsilon=1\) and \(r=-n\). The exact
...
\[
 \frac1a+\frac1b=\frac{n-r}{2n}\ge\frac12.
\]
...
Both components have order greater than \(28\). Since their total excess is one,
the classification forces one complete component and one cocktail-party
...
\(61\), contradicting divisibility of the corresponding rational primary
component by four.
...
\[
 3(4x-1),\qquad 6(3x-1),\qquad 9(2x-1)
\]
...
\[
 ...
 \qquad
 |V(Y)|=\frac{2n}{q}<\frac{32}{3}.
\]

Simplicity leaves only \((q,|V(Y)|)=(8,10)\) or \((9,10)\), giving
\((n,r)=(40,8)\) or \((45,9)\). The radius-two bound forces \(k=6\), and the
exact formula for \(r\) rules out both.

If \(Y\) is semiregular bipartite with part sizes \(a\ge b\ge2\), then

\[
 \frac1a+\frac1b=\frac{n-r}{2n}\in\left(\frac38,\frac12\right).
\]
...
order. Thus every component is complete or cocktail-party. Their excesses over
\(d+1\) are \(0\) and \(1\), so \(a_1+a_2\le2\), contradicting
\(a_1+a_2=r+2>2\).
