...
\[
 ...
 \right\rfloor;
\]
its principal minors recover local cycle constraints and imply sharper
low-degree windows.  In particular, degree-six counterexamples have order at
most \(50\).  We also determine the distance spectra of one- and two-vertex
...
is the LP defect, while integrality gives
\[
 n\le\left\lfloor
 \frac{(k+2)^2(k^2+3)}{6(k+2)+1}
 \right\rfloor;
\]
see Theorem~\ref{thm:integral-slack}.  Its \(2\times2\) minors yield a general
cycle-divisibility sieve and the local order-\(51\) contradiction
\[
 5N_5=153\cdot11=1683,
\]
while the next integral level gives the sharper degree-\(7,8,9\) order
windows \(74,108,150\); see
Corollary~\ref{cor:low-degree-windows}.
...
optimal slack matrix strengthens the order bound; \(2\times2\) principal
minors localize that certificate to edge--cycle counts; and \(3\times3\)
minors impose the two-path constraints at order fifty.  In the stability
...
Theorem~\ref{thm:integral-slack} gives
\[
...
Every regular strict counterexample in the indicated degrees satisfies
\begin{align*}
k=5&:\quad n\le30,\\
k=6&:\quad n\le50,\\
...
\begin{proof}
Theorem~\ref{thm:diameter-four} removes diameter four.  The integral bound
gives, for \(k=5,6,7,8,9\), respectively,
\[
 n\le31,\ 50,\ 76,\ 109,\ 151.
\]
The equations \(5n=2|E(G)|\) and \(9n=2|E(G)|\) improve the first and last
bounds to \(30\) and \(150\).  The diameter-two alternatives are determined
by the Moore multiplicities: only the degree-seven, order-\(50\)
Hoffman--Singleton case survives.

It remains to exclude \((k,n)=(7,76)\) and \((8,109)\) in diameter three.
In either case the upper half of the \(2\times2\) estimate in the proof of
Theorem~\ref{thm:integral-slack} gives
\[
...
\]
Hence \(\mathcal E_k\) is the adjacency matrix of a regular graph \(Y\).
...
\]
In both cases \(d_X>(|V(X)|-2)/2\), so \(X\) is connected.  The
Cameron--Goethals--Seidel--Shult classification
\cite{CameronEtAl1976,KoolenEtAl2025} now implies that \(X\) is a line graph
or a cocktail-party graph.  Its degree is not \(|V(X)|-2\), so the latter is
impossible.

Write \(X=L(Z)\).  Since \(X\) is regular and connected, the degree sum across
every edge of \(Z\) is constant; hence \(Z\) is regular or semiregular
bipartite.  For \((76,42)\), a regular root would have degree \(22\), but
\[
 |V(Z)|=\frac{2\cdot76}{22}=\frac{76}{11}\notin\mathbb Z.
\]
A semiregular bipartite root would require two divisors of \(76\) summing to
\(44\), and no such pair exists.  For \((109,56)\), a regular root would
have degree \(29\), but \(29\nmid218\); a semiregular root would require two
divisors of the prime \(109\) summing to \(58\), again impossible.  Therefore
\(n\ne76,109\).  Since \(7n\) is even, the degree-seven diameter-three bound
improves to \(74\).  All arithmetic after the cited classification theorem has
also been checked exactly.
\end{proof}
...
\(n\le51\) and \(n\le50\), the integral optimal-slack theorem, the sharper
low-degree windows, and the local Gram hierarchy remain conventional analytic
proofs, with exact executable audits where cited.  The punctured-Moore
