...
\[
 ...
 \right\rfloor;
\]
this follows from a four-to-one quantization theorem for the integral excess.
The slack matrix's principal minors also recover local cycle constraints.  In
particular, degree-six counterexamples have order at most \(50\).  We determine
the distance spectra of one- and two-vertex punctures of Moore graphs and
establish a uniform deletion-stability bound:
...
is the LP defect.  Its integral excess satisfies a four-to-one quantization
inequality, giving
\[
 n\le\left\lfloor
 \frac{8(k+2)^2(k^2+3)}{48k+109}
 \right\rfloor;
\]
see Theorems~\ref{thm:integral-slack} and~\ref{thm:four-to-one}.  Its
...
small Gram minors quantize the remaining excess into the four-to-one bound;
edge-local \(2\times2\) minors convert the same certificate into cycle counts;
and \(3\times3\) minors impose the two-path constraints at order fifty.  In
the stability
...
\begin{theorem}[Four-to-one excess bound]\label{thm:four-to-one}
...
cocktail-party graph \cite{CameronEtAl1976,KoolenEtAl2025}.
...
\(g_{44}(x)+2\).  This quartic is irreducible modulo \(11\), so that
dimension must be divisible by four, a contradiction.
...
has dimension \(332373\), whereas \(g_{158}(x)+2\) is irreducible modulo
\(23\), again a contradiction.
...
multiplicity \(10219\).  The quartic \(g_{62}(x)+2\) is irreducible modulo
\(61\), contradicting divisibility of this rational primary component by
four.  Therefore \(r>0\).

\smallskip
\noindent\emph{The four-to-one inequality.}
Assume for contradiction that \(n>4r\), and put \(x=r/n\in(0,1/4)\).
The \(2\times2\) minors now give \(E_{uv}\le2\).  Suppose
\(E_{uv}=2\), and for \(w\notin\{u,v\}\) set
\(s_w=E_{uw}+E_{vw}\).  Cauchy--Schwarz in the Gram matrix
\(\mathcal S_k\) gives
\[
 (1+x-s_w)^2\le x(3+x),
\]
so \(s_w\in\{1,2\}\).  The row sums show that exactly \(r\) vertices have
\(s_w=2\).  A vertex of type
\((E_{uw},E_{vw})=(2,0)\) or \((0,2)\) would give the negative principal
minor
\[
 \frac{11x-3}{2}<0;
\]
hence all \(r\) exceptional vertices have type \((1,1)\).  If two such
vertices \(w,z\) existed, the corresponding \(4\times4\) determinant would
be
\[
 3(4x-1),\quad6(3x-1),\quad\text{or}\quad9(2x-1),
\]
according as \(E_{wz}=0,1,2\).  Each is negative.  Thus \(r\le1\), while
\eqref{eq:r-divisibility} excludes \(r=1\).  Consequently \(E\) is simple.

Again \(X=\overline E\) is \(d\)-regular with least eigenvalue at least
\(-2\).  Three components would imply \(n\le3r+6\), incompatible with
\(n>4r\), \(r\ge1\), and \(n\ge38\).  If \(X\) is connected and
\(X=L(Y)\) for a \(q\)-regular graph \(Y\), then
\[
...
\]
The radius-two bound forces \(k=6\), and the defining formula for \(r\)
excludes both cases.
...
\]
Exact substitution in the formula for \(r\), subject to \(n\ge k^2+2\)
and \(kn\) even, excludes every entry.

It remains to consider two components.  For \(k=6,7,8\), the inequalities
\(r>0\) and \(n>4r\) contain no admissible integral order.  For \(k\ge9\),
both components have order greater than \(28\).  A regular line-graph root
would have degree greater than \(3n/16\) and order less than seven, which is
impossible.  For a semiregular bipartite root, component size gives
\[
 \frac1a+\frac1b>\frac7{12}.
\]
The possible part sizes have \(ab\le22\), smaller than the component order.
The classification therefore leaves only complete and cocktail-party
components.  Their respective excesses over \(d+1\) are \(0\) and \(1\), so
\(a_1+a_2\le2\), contradicting \(a_1+a_2=r+2>2\).

We conclude that \(n\le4r\).  Substituting the definition of \(r\) gives
\[
 (48k+109)n\le8(k+2)^2(k^2+3),
\]
which is the displayed bound.  All polynomial divisions, modular
irreducibility certificates, finite line-root cases, and Gram determinants
in this argument have been independently checked in exact arithmetic.
\end{proof}
...
Theorem~\ref{thm:four-to-one} gives
\[
...
For diameter three, Theorem~\ref{thm:four-to-one} gives, for
...
row sum \(2\).  Thus the unresolved degree-six boundary is equivalently a
...
four-to-one excess bound, the sharper low-degree windows, and the local Gram
