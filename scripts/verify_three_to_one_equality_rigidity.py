...
def main() -> None:
    ...
    if admissible_moduli != [1895]:
        raise AssertionError(f"wrong admissible equality moduli: {admissible_moduli}")

    degree = (admissible_moduli[0] - 41) // 18
    if degree != 103:
        raise AssertionError("wrong equality degree")
    c_integer = int(c_value.subs(K, degree))
    excess = c_integer // admissible_moduli[0]
    order = 3 * excess
    if (c_integer, excess, order) != (116997300, 61740, 185220):
        raise AssertionError("wrong equality parameter triple")
    if degree * order % 2:
        raise AssertionError("the unique arithmetic equality case violates handshake parity")

    # Conversely, verify the defining equality exactly.
