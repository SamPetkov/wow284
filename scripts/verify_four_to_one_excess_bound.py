...
def nonpositive_r_audit() -> dict[str, object]:
    ...
    if not irreducible_mod(g62 + 2, 61):
        raise AssertionError("g_62+2 lacks the mod-61 irreducibility certificate")
    if (b62 // 2) % 4 == 0:
...
def doubled_edge_gram_audit() -> dict[str, str]:
    ...
    if sp.expand(cauchy_rhs - x * (3 + x)) != 0:
    ...
    if sp.expand(det3(2, 0) - (11 * x - 3) / 2) != 0:
        raise AssertionError("wrong asymmetric three-vertex determinant")
    if sp.expand(det3(1, 1) - 3 * (5 * x - 1) / 2) != 0:
    ...
    if any(sp.expand(actual[weight] - expected[weight]) != 0 for weight in expected):
