...
now follows from Theorem~\ref{thm:moore-threshold}.  At \(k=7\), the
...
\(r,s=(-1\pm\sqrt{4K-3})/2\) on \(\ker C\).  Moreover,
\[
 \dim\ker C=K(K-2),\qquad \tr(B|_{\ker C})=0.
\]
Indeed, the principal eigenvalue \(K-1\) cancels the \(K-1\) copies of
\(-1\) in the trace.  Hence the root multiplicities \(p,q\) satisfy
\[
...
\]
Thus every ordered eigenvalue of \(Q(a)\) is nondecreasing in \(a\).
...
\]
Because the leading coefficient is positive, the largest root \(\mu_2\) of
the nonprincipal factor exceeds \(11/6>-1+\sqrt8\).  Cauchy interlacing gives
\(\theta_2(A)\ge\mu_2\), contradicting the necessary bound
\(\theta<-1+\sqrt8\).  At \(n=30\), Meringer's isomorph-free enumeration
leaves exactly four \((5,5)\)-cages \cite[p.~142]{Meringer1999}; each fixed record has
...
subgraph of edges that lie in \(13\) five-cycles, let \(m=|E(H)|\), and let
\(\tau(v)\) be
...
\]
 ...
 \]
 Thus the unresolved degree-six boundary can therefore be viewed as a
...
type \(D_\ell\) or \(E_8\).  The rank bound excludes \(E_8\).  Thus
\[
B^{\mathsf T}B=T+2I
\]
...
\]
The cell-constant space, the two-dimensional spaces generated by
\((z,0)\) and \((0,C^{\mathsf T}z)\) for \(z\in\one_A^\perp\), and
\(0\oplus K\) are mutually orthogonal and invariant.  Denote the first space
by \(W_{\mathrm{const}}\) and the sum of the incidence spaces by
\(W_{\mathrm{inc}}\).  Consequently
\[
...
\(-2-\sqrt{k}\).  The same is true of the smaller constant-quotient roots:
...
For \(x\in\one_A^\perp\), the maps
\(x\mapsto(R_A^{\mathsf T}\pm R_B^{\mathsf T})x\), and for
\(y\in\one_C^\perp\), the map \(y\mapsto R_C^{\mathsf T}y\), are injective
with mutually orthogonal images.
Together with the cell-constant space and
\[
...
\]
on the matched symmetric, antisymmetric, and common-neighbour modules,
respectively.  These give the two displayed quadratic factors and \(k-3\)
copies of each Moore linear factor.  On \(K\),
\[
 T^2+T-(k-1)I=0,
 \qquad D=-2I-T.
\]
...
the three negative terms in \(f'(x)\) have sum less than
...
polynomial and at coefficient level.  The public Lean development is
