...
\section{Scope and open directions}\label{sec:scope}

Classical inputs are cited where they first enter the argument.  The contribution
of this paper is the combined mechanism developed above: the shifted spectral
score, the exact LP ceiling and its local semidefinite refinements, the puncture
spectra, and the deletion-stability theory.  The natural next questions concern
how far these mechanisms extend.

\begin{enumerate}
\item What are the minimum order and possible degree sequences of strict
counterexamples, and can the extremal examples be classified?
\item Can a systematic multipoint semidefinite or higher-moment hierarchy
extend the exact one-point LP and local-minor bounds, in particular at the
degree-six order-\(50\) boundary?
\item What replaces the diameter-three score calculus for irregular graphs and
for regular graphs of diameter four, and does either setting produce new
counterexamples?
\item Does an unconditional infinite family of strict counterexamples exist,
and which graph operations preserve strictness?  Any infinite regular family
must have unbounded degree.
\item Can the puncture and metric-damage theory be extended from Moore graphs
to broader distance-regular or distance-polynomial families, with sharp
...
and correspond to release \texttt{v2.2.3}.
