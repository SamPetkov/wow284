...
\[
 ...
 =2k-2-\max_{\theta\ne k}(\theta+1)^2,
\]
where \(\theta\) ranges over the nonprincipal adjacency eigenvalues.  Every
regular strict counterexample has degree at least six and diameter at most
four; the diameter-four regime starts only at degree ten.  We determine the
exact optimum, and the unique optimizer up to scale, of the standard
one-variable nonbacktracking linear-programming bound.  Localizing its
extremal polynomial at an edge yields a five-cycle certificate which excludes
degree-six order \(51\), and hence every degree-six regular strict
counterexample has order at most \(50\).  We further derive complete distance
spectra for one- and two-vertex punctures of Moore graphs, prove a general
deletion-stability inequality, and give a metric normal form for arbitrary
punctures of size at most \(k-1\).  In particular, every deletion of at most
five vertices from the Hoffman--Singleton graph remains a strict
counterexample, while one explicit six-vertex deletion does not.  Equality
cases, finite-field obstructions, matching-deletion controls, and exact local
constraints on a hypothetical degree-six order-50 example are also obtained.
...
positive-semidefinite certificate then rules out \(n=51\) by the congruence
\[
...
\(\diam(G)<k\).  The cases \(k\le2\) are immediate.  For \(k=3\), the girth
...
counterexample, each factor \(2k-2-y_i^2\) is positive.  The sum cannot vanish,
because that would force every nonprincipal adjacency eigenvalue to be \(-2\),
contradicting the trace equation.  The identity is checked symbolically by
...
Assume \(n=51\), so \(c=14\).  Proposition~\ref{prop:edge-cycle} gives, for
every edge,
\[
 11\le\sigma_{uv}\le\frac{202}{17}<12.
\]
Hence \(\sigma_{uv}=11\).  The graph has \(153\) edges, and counting incidences
between edges and \(5\)-cycles gives
\[
 5N_5=153\cdot11=1683,
\]
...
\]
Two explicitly stored permutations are verified edge-by-edge as automorphisms
of the coordinate graph; the group they generate has order \(252000\).  Their
orbits on deletion sets of sizes \(0,1,2,3,4,5\) have counts
\[
 1,1,2,4,11,33.
\]
...
\(m\in\{4,5,6\}\).  A nonzero character block has form
\[
...
\[
 \mathcal M_\pi=\{P_{i,j}Q_{\pi(i),i\pi(i)+j}:i,j\in\F\}
\]
...
The release audit rejects \texttt{sorry}, \texttt{admit},
\texttt{native\_decide}, \texttt{bv\_decide}, unsafe declarations, and new
axioms.  Representative axiom reports contain only
\texttt{propext}, \texttt{Classical.choice}, and \texttt{Quot.sound}.

\section{Scope, literature boundary, and open questions}\label{sec:scope}

Several ingredients belong to established theory and should not be conflated
with the project-derived statements.

\begin{itemize}
\item Moore graphs, the Hoffman--Singleton graph, and their intact adjacency and
distance spectra are classical \cite{HoffmanSingleton1960,HowladerPanigrahi2022}.
\item Proposition~\ref{prop:radius-two} is Backelin's second-degree identity in
normalized form \cite{Backelin2015}.
\item Proposition~\ref{prop:higher-transfer} belongs to the established
distance-polynomial framework \cite{Fiol2016}.
\item The one-variable nonbacktracking LP framework is due to Nozaki
\cite{Nozaki2015}; Theorem~\ref{thm:lp-ceiling} is the exact two-sided WOW
specialization and optimizer theorem derived here.
\item The four \((5,5)\)-cages used in Theorem~\ref{thm:regular-degree-six}
come from Meringer's published exhaustive generation \cite{Meringer1999}; the
repository independently verifies the four fixed records but does not replace
the enumeration.
\item Literature on vertex-deleted adjacency spectra and subconstituents is
close in language but does not directly give the recomputed ordinary distance
spectra in Section~\ref{sec:punctures}
\cite{DalfoVanDamFiol2012,Biggs2010}.
\end{itemize}

Targeted searches, recorded in
\codefile{research-notes/LITERATURE_AUDIT_EXTENSIONS.md} and the associated
source matrix, did not locate direct prior statements of the punctured-Moore
distance spectra, the exact two-sided LP optimizer, the edge-local WOW cycle
inequality, or the endpoint-neighbourhood bound.  Search silence is not proof
...
\begin{enumerate}
\item Is there a counterexample of order below \(38\), and what is the true
minimum order?
\item Does a \(6\)-regular order-\(50\) strict counterexample exist?
\item Can the one-point and three-vertex constraints of
Theorem~\ref{thm:order50-feasibility} be completed by a multipoint semidefinite
or canonical-generation argument?
\item Do regular diameter-four counterexamples exist for degree at least ten?
\item Is there an unconditional infinite family of strict counterexamples?
Any infinite regular family must have unbounded degree, since
Corollary~\ref{cor:trichotomy} and the Moore bound leave finitely many graphs
at each fixed degree.
\item Which deleted sets in a general Moore graph preserve strictness, beyond
the metric normal form of Theorem~\ref{thm:small-puncture}?
\end{enumerate}

OpenAI ChatGPT-5.6 Sol Pro assisted with adversarial proof checking, literature
search, exact verification code, and Lean formalization.  No AI system is an
...
and correspond to release \texttt{v2.2.0}.

\appendix

\section{Characteristic polynomials for the smallest explicit descendants}
