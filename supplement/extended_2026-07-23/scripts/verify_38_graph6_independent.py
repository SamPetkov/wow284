...
This pass starts only from graph6, uses NetworkX Floyd--Warshall for distances,
...
def main() -> None:
    ...
    # Exact integer distances from an algorithm independent of the coordinate BFS.
    distances = nx.floyd_warshall_numpy(graph, nodelist=range(38)).astype(int)
    ...
    x = sp.symbols("x")
    distance_matrix = sp.Matrix(distances.tolist())
    ...
    shifted = (3 * distances + 17 * sp.eye(38)).tolist()
